...
//        }
        ...
        if (!mysqli_query($conn, "UPDATE customerOrder SET orderTotalAmount=$total WHERE orderID=$oid")) $dbError=true;
    }

    // 更新状态
    if (!mysqli_query($conn, "UPDATE customerOrder SET orderStatu=$newStatus WHERE orderID=$oid")) $dbError=true;

    if (!$dbError) {
        mysqli_query($conn, "COMMIT");
        // 重新读最新总额返回给前端
        $newTotalRes = mysqli_query($conn, "SELECT orderTotalAmount FROM customerOrder WHERE orderID=$oid");
